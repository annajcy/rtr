...
const char* mvp_vertex_shader_source = R"(
#version 460 core
...
const char* phong_fragment_shader_source = R"(
#version 460 core
out vec4 FragColor;
...
void main()
{
    ...
    localNormal = localNormal * 2.0 - vec3(1.0);
...
const char* screen_vertex_shader_source = R"(
#version 460 core
...
const char* screen_fragment_shader_source = R"(
#version 460 core
out vec4 FragColor;
in vec2 TexCoords;

uniform sampler2D screentTexture;

void main()
{
    vec4 color = texture(screentTexture, TexCoords);
    FragColor = color; 
}
)";
...
Image::Ptr main_image{};
Image::Ptr normal_image{};

Image::Ptr back_image{};
Image::Ptr front_image{};
Image::Ptr left_image{};
Image::Ptr right_image{};
Image::Ptr top_image{};
Image::Ptr bottom_image{};


void prepare_resource() {
    back_image = Image_loader::load_from_path(
        Image_format::RGB_ALPHA,
        "assets/image/skybox/back.jpg",
        false
    );
    front_image = Image_loader::load_from_path(
        Image_format::RGB_ALPHA,
        "assets/image/skybox/front.jpg",
        false
    );
    left_image = Image_loader::load_from_path(
        Image_format::RGB_ALPHA,
        "assets/image/skybox/left.jpg",
        false
    );
    right_image = Image_loader::load_from_path(
        Image_format::RGB_ALPHA,
        "assets/image/skybox/right.jpg",
        false
    );
    top_image = Image_loader::load_from_path(
        Image_format::RGB_ALPHA,
        "assets/image/skybox/top.jpg",
        false
    );
    bottom_image = Image_loader::load_from_path(
        Image_format::RGB_ALPHA,
...
RHI_device::Ptr device{};
RHI_window::Ptr window{};
RHI_imgui::Ptr imgui{};
Input::Ptr input{};

void init_global() {
    device = RHI_device_OpenGL::create();
    window = device->create_window(1280, 720, "RTR");
    imgui = device->create_imgui(window);

    input = Input::create(window);
}

RHI_shader_program::Ptr skybox_shader{};
RHI_shader_program::Ptr normal_shader{};
RHI_shader_program::Ptr screen_shader{};

void prepare_shader_program() {
    skybox_shader = device->create_shader_program(
        std::unordered_map<Shader_type, RHI_shader_code::Ptr> {
            {Shader_type::VERTEX, device->create_shader_code(Shader_type::VERTEX, cubemap_vertex_shader_source)},
            {Shader_type::FRAGMENT, device->create_shader_code(Shader_type::FRAGMENT, cubemap_fragment_shader_source)}
        }, 
        std::unordered_map<std::string, RHI_uniform_entry_base::Ptr> {
            ...
            {"screenTex", RHI_uniform_entry<int>::create(0)}
        }
    );

    normal_shader = device->create_shader_program(
    	std::unordered_map<Shader_type, RHI_shader_code::Ptr> {
            {Shader_type::VERTEX, device->create_shader_code(Shader_type::VERTEX, mvp_vertex_shader_source)},
            {Shader_type::FRAGMENT, device->create_shader_code(Shader_type::FRAGMENT, phong_fragment_shader_source)}
        }, 
        [&](){
            ...
            std::unordered_map<std::string, RHI_uniform_entry_base::Ptr> uniforms{
                ...
                {"normalTexture", RHI_uniform_entry<int>::create(1)},
    ...
    screen_shader = device->create_shader_program(
        std::unordered_map<Shader_type, RHI_shader_code::Ptr> {
            {Shader_type::VERTEX, device->create_shader_code(Shader_type::VERTEX, screen_vertex_shader_source)},
            {Shader_type::FRAGMENT, device->create_shader_code(Shader_type::FRAGMENT, screen_fragment_shader_source)}
        },
        std::unordered_map<std::string, RHI_uniform_entry_base::Ptr> {
            {"screentTexture", RHI_uniform_entry<int>::create(0)}
        }
    );
}
...
int main() {
    ...
    auto vertex_shader_code = device->create_shader_code(Shader_type::VERTEX, mvp_vertex_shader_source);
    auto fragment_shader_code = device->create_shader_code(Shader_type::FRAGMENT, phong_fragment_shader_source);
